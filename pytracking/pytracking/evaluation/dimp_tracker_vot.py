import sys
sys.path.append("/home/tong/tracker/pytracking")
...
# sys.path.append('/home/tong/tracker/maskrcnn-benchmark')
# from demo.predictor import *
# from maskrcnn_benchmark.config import cfg

# # sys.path.append('./fastreid')
# sys.path.append('/home/tong/tracker/fast-reid')
...
# detector_config = "/home/tong/tracker/maskrcnn-benchmark/configs/caffe2/e2e_mask_rcnn_R_50_FPN_1x_caffe2.yaml"
# reid_config = "/home/tong/tracker/reid.yml"

# # update the config options with the config file
# cfg.merge_from_file(detector_config)

# detector = COCODemo(cfg, min_image_size=200, confidence_threshold=0.5)
...
class Tracker:
    ...
    def run_video_no_display(self, vot, detector, reid_predictor, videofilepath, output_dir, optional_box=None, debug=None, visdom_info=None, save_results=False):
        """Run the tracker with the videofile.
        args:
            debug: Debug level.
        """
        
        vot_handle = vot.VOT("rectangle")
...
#             output_boxes.append(optional_box)
        ...
        while True:
            ...
            if frame_count == 0:
                ...
                selection = vot_handle.region()
                optional_box = [selection.x, selection.y, selection.width, selection.height]
                optional_box = list(map(int, optional_box))
                print("optional_bbox: ", optional_box)
                ...
                target_object = frame[optional_box[1]:(optional_box[1]+optional_box[3]), optional_box[0]:(optional_box[0]+optional_box[2]), :]
                target_object = target_object[:, :, ::-1]
                # Apply pre-processing to image.
                reid_input_size = params.reid_input_size
                image = cv2.resize(target_object, (reid_input_size, reid_input_size), interpolation=cv2.INTER_CUBIC)
                cv2.imwrite("target.jpg", image)
                # Make shape with a new batch dimension which is adapted for
                # network input
                image = torch.as_tensor(image.astype("float32").transpose(2, 0, 1))[None]
                predictions = reid_predictor(image)
                self.target_reid_feature.append(torch.squeeze(predictions).numpy())
                frame_count += 1
...
#               'flag': flag}
            ...
            print("state: ", state)
            max_score = out["max_score"][1]
            final_result = [state[1], state[0], state[3], state[2]]
            final_confidence = min(1, max_score)
            # print("max_score: ", max_score)
            flag = out["flag"][1]
            image_patch = torch.squeeze(out['im_patches'][1].cpu())
            sample_coords = out["sample_coords"][1]
            sample_coords = torch.squeeze(sample_coords).cpu().numpy()
            ...
            if params.has('detector_freq'):
                ...
                # if (frame_count + 1)%params.detector_freq == 0 or flag == "uncertain" or flag == "hard_negative":
                # if True:

                    detector_result = detector.compute_prediction(image_patch.numpy().transpose(1, 2, 0).astype(np.uint8))
                    
                    if len(detector_result.get_field("scores")) > 0:
                        # cv2.imwrite("search_area.jpg", image_patch.numpy().transpose(1, 2, 0).astype(np.uint8))

                        # Process detector result, add window penalty
                        detector_bboxes = detector_result.bbox.numpy()
                        bbox_centers = [[(bbox[0] + bbox[2])/2, (bbox[1] + bbox[3])/2] for bbox in detector_bboxes]
                        # print(detector_result.bbox)
                        detection_conf = detector_result.get_field("scores")
                        windowed_score = [detection_conf[ii]*self.penalty_window[int(bbox_c[0]), int(bbox_c[1])] for ii, bbox_c in enumerate(bbox_centers)]
                        # rint("windowed_score: ", windowed_score, detection_conf)
                        # cv2.imwrite("window.jpg", self.penalty_window*255)

                        rescaled_detections = np.array(detector_result.bbox)*np.array(sample_size_original_image*2)[np.newaxis, :]/params.image_sample_size + np.array([sample_coords[0], sample_coords[1], sample_coords[0], sample_coords[1]])[np.newaxis, :]
                        rescaled_detections = rescaled_detections.astype(np.int32)
                        
                        # Clip rescaled detections into proper range: [0, max_image_size]
                        frame_size = frame.shape[:2]
                        # print("frame_size:", frame_size)
                        rescaled_detections = np.clip(rescaled_detections, 0, np.array(frame_size*2)[np.newaxis, :])
                        print("rescaled_detections: ", rescaled_detections)
                        image_crops = []
                        for bbox in rescaled_detections:
                            if bbox[0] >= bbox[2] or bbox[1] >= bbox[3]:
                                image_crops.append(np.zeros((reid_input_size, reid_input_size, 3)))
                            else:
                                image_crops.append(cv2.resize(frame[bbox[0]:bbox[2], bbox[1]:bbox[3], :], (reid_input_size, reid_input_size)))

                        if flag == "normal":
                            image_crops.append(cv2.resize(frame[state[1]:(state[3]+state[1]), state[0]:(state[2]+state[0]), :], (reid_input_size, reid_input_size)))

                        cv2.imwrite("sample.jpg", image_crops[-1])
                        image_crops = np.array(image_crops)[:, :, :, ::-1].astype(np.float32) # BGR to RGB
                        image_crops = torch.as_tensor(image_crops.transpose(0, 3, 1, 2))
                        embeddings = reid_predictor(image_crops).numpy()
                        if flag == "normal":
                            similarity_score = np.dot(embeddings[:-1, :], np.array(self.target_reid_feature).T).mean(axis=1)
                        else:
                            similarity_score = np.dot(embeddings, np.array(self.target_reid_feature).T).mean(axis=1)
                        # print(similarity_score)
                        overall_score = windowed_score * similarity_score

                        index = np.argmax(overall_score)
                        max_detection_score = overall_score[index]
                        # print("best_detection_bbox: ", rescaled_detections[index])

                        if max_detection_score > max_score: 
                            # Update tracker result with detector result
                            print("detector result accepted!: ", state, rescaled_detections[index])
                            best_detection_bbox = rescaled_detections[index]
                            final_result = [best_detection_bbox[0], best_detection_bbox[0], best_detection_bbox[2] - best_detection_bbox[0], best_detection_bbox[3] - best_detection_bbox[1]]
                            final_confidence = max_detection_score
                            tracker.update([torch.tensor([(best_detection_bbox[0] + best_detection_bbox[2])/2, (best_detection_bbox[1] + best_detection_bbox[3])/2])])

                        if flag == "normal":
                            self.target_reid_feature.append(embeddings[-1, :])
                            if len(self.target_reid_feature) > params.reid_feature_budget:
                                # if exceed budget, pop second reid feature and keep template feature
                                self.target_reid_feature.pop(1)

                        for bbox in rescaled_detections: 
                            cv.rectangle(frame_disp, (bbox[1], bbox[0]), (bbox[3], bbox[2]), (255, 0, 0), 1)
            ...
            if self.frames_since_lost > params.max_lost_frames_redetection:
                ...
                if len(detector_result.get_field("scores")) > 0:
                    detector_bbox_int = detector_result.bbox.numpy().astype(np.int32)
                    
                    detector_bbox_int = np.clip(detector_bbox_int, 0, np.array(frame_size*2)[np.newaxis, :])
                    
                    image_crops = []
                    for bbox in detector_bbox_int:
                        if bbox[0] >= bbox[2] or bbox[1] >= bbox[3]:
                            image_crops.append(np.zeros((reid_input_size, reid_input_size, 3)))
                        else:
                            image_crops.append(cv2.resize(frame[bbox[0]:bbox[2], bbox[1]:bbox[3], :], (reid_input_size, reid_input_size)))
                    ...
                    detection_conf = detector_result.get_field("scores").numpy()
                    redetection_score = similarity_score * detection_conf

                    # If re-detection result is confident enough, update or reinitialize tracker
                    if max(redetection_score) > params.redetection_accept_threshold: 
                        index = np.argmax(redetection_score)
                        max_detection_score = redetection_score[index]
                        best_redetection_bbox = detector_result.bbox[index]
                        final_confidence = max_detection_score
                        final_result = [best_redetection_bbox[0], best_redetection_bbox[0], best_redetection_bbox[2] - best_redetection_bbox[0], best_redetection_bbox[3] - best_redetection_bbox[1]]
                        tracker.update([torch.tensor([(best_redetection_bbox[0] + best_redetection_bbox[2])/2, (best_redetection_bbox[1] + best_redetection_bbox[3])/2])])
            ...
            font_color = (0, 0, 0)
            cv.putText(frame_disp, flag, (20, 30), cv.FONT_HERSHEY_COMPLEX_SMALL, 1,
                       font_color, 1)
            final_result = vot.Rectangle(x=final_result[1], y=final_result[0], width=final_result[3], height=final_result[2])
            vot_handle.report(final_result, final_confidence)
